...
- 修复打招呼成功后目标岗位仍留在打招呼页的问题：批量发送成功、人工确认成功后都会从待处理目标中移除岗位，同时保留岗位状态和发送历史。
- 增加历史数据兼容清理：应用启动或重新加载打招呼目标时，会自动清理已经标记为 `sent` 的遗留岗位，避免旧版本残留目标重复显示。
- 修复打招呼页面脚本异常诊断：保留原始 JavaScript 异常、失败阶段、当前 URL 和 CDP 返回状态，便于定位“立即沟通后未发送”的具体断点。
- 增加模块切换自动刷新：进入仪表盘、简历、岗位、尽调、排序或打招呼模块时自动加载一次最新数据，隐藏模块不再重复轮询。
- 尽调、排序、打招呼页面保留手动刷新入口，作为实时数据异常时的主动刷新手段。
