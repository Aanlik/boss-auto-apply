...
def help_center() -> dict:
    modules = [
        ...
        },
        {
            ...
            "commonFailures": ["话术校验失败", "页面风控", "找不到立即沟通按钮", "未先通过灰度首发", "连续失败后自动发送按钮变灰"],
            "safetyNotes": ["自动发送不绕过验证码和风控，出现风险提示时应暂停并人工处理。", "连续失败达到安全阈值后会锁定自动发送；先完成预检，再手动成功发送 1 个岗位，并在该岗位卡片点击“标记已招呼”后刷新安全阈值。"],
            "repairActions": [
